...
3. **Setup**: Add `ALPHA_VANTAGE_API_KEY` to `.env`. Configure `DATABASE_URL` if using Postgres. Ensure Redis is running locally or set `REDIS_URL`.
4. **Health Check**: Run `python sovereign-cli.py health`.
5. **Start Workers**: Start Celery workers and beat scheduler:
   ```bash
   celery -A worker.celery_app worker --loglevel=info --pool=solo -c 4
   celery -A worker.celery_app beat --loglevel=info
   ```
6. **Scan**: Execute `python main.py` and access the dashboard at `:9005`.
...
**Note for New Checkouts:** If you are running the API or tests on a truly fresh checkout, you must run the screener pipeline (`python screener.py --smoke`) at least once first, or call `database.init_db()` manually, so that the initial `stocks.db` database and its tables are created. Otherwise, you may encounter "no such table" errors.

**Note for Contributors:** The `web-ui/node_modules` folder is strictly gitignored. You must run `cd web-ui && npm install` before running the frontend.

Generated reports (e.g. `reports/` and `reports_cache/`) are also gitignored to prevent repo bloat.
