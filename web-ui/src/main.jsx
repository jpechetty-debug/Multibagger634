...
function NavBar({regime, vix, liveCount, connected, activeTab, onTabChange}) {
  ...
  return (
    <nav className="h-14 flex items-center px-4 border-b border-[#1a2b3c] bg-[#041627] shadow-sm">
      <div className="flex items-center pr-6 border-r border-[#1a2b3c] h-full">
        <svg className="w-8 h-8 text-[#00e5ff] mr-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        {tabs.map(t => (
          <button 
            ...
            className={`h-full relative px-1 text-xs font-semibold whitespace-nowrap transition-colors bg-transparent border-none ${
              activeTab === t ? 'text-[#00e5ff]' : 'text-slate-400 hover:text-white'
            }`} 
            onClick={() => onTabChange(t)}
          >
            {t}
            {activeTab === t && (
              <div className="absolute bottom-0 left-0 w-full h-[2px] bg-[#00e5ff] rounded-t-sm"></div>
            )}
          </button>
        ))}
      </div>

      <div className="flex items-center pl-6 border-l border-[#1a2b3c] h-full gap-4">
        ...
        <button className="w-7 h-7 bg-[#1a2b3c] rounded-full flex items-center justify-center text-slate-400 hover:text-white transition-colors border-none">
...
function TickerTape({stocks}) {
  ...
  return (
    <div className="flex items-center h-8 bg-[#041627] border-b border-[#1a2b3c] text-[11px] font-mono overflow-hidden whitespace-nowrap text-white relative">
      <div className="flex items-center justify-center w-8 h-full border-r border-[#1a2b3c] shrink-0 bg-[#041627] z-10 text-slate-400 absolute left-0 top-0">
      ...
      <div className="ticker-scroll flex items-center pl-8">
        {doubled.map((s, i) => {
          ...
          return (
            <div className="px-4 border-r border-[#1a2b3c] flex gap-2 items-center" key={i}>
              ...
              <span className={`font-semibold ${chg >= 0 ? 'text-[#00e5ff]' : 'text-rose-400'}`}>{chg >= 0 ? '+' : ''}{chg.toFixed(1)}%</span>
            </div>
          );
        })}
      </div>
