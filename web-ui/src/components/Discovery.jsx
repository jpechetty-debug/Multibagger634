...
export default function Discovery({ onSelectStock }) {
  ...
  const renderSection = (title, items, formatter) => (
    <div className="border border-brand-border bg-brand-card rounded-xl flex flex-col h-64">
      ...
      <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
        {items.map((item, i) => (
          <div key={i} onClick={() => onSelectStock(item.symbol || item)} className="flex justify-between items-center py-2 px-2 border-b border-white/5 hover:bg-white/5 cursor-pointer rounded">
             {formatter(item)}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-6">
      <div className="text-xl font-bold text-white mb-4">Discovery Engine</div>
