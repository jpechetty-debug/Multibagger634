...
export default function ExplainabilityDrawer({ stock, onClose }) {
  ...
  return (
    <div className="w-96 border-l border-brand-border bg-brand-base flex flex-col h-full absolute right-0 top-0 z-50 transition-transform duration-300">
      {/* Header */}
      <div className="p-4 border-b border-brand-border flex justify-between items-start bg-brand-card">
        <div>
          <div className="text-3xl font-bold text-slate-300">{stock}</div>
          {data ? (
             <div className="flex gap-2 mt-2">
                <span className="px-2 py-1 bg-brand-accent/10 text-brand-accent text-xs font-mono">MB Score: {data.score}</span>
                <span className="px-2 py-1 bg-white/5 text-slate-300 text-xs font-mono">Opp: {data.opportunity_score}</span>
             </div>
      ...
      {data && (
        <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
          ...
          <div className="border border-brand-border bg-brand-card overflow-hidden">
            ...
            <div className="p-3">
              <div className="text-lg font-bold text-slate-300 mb-3">Rank #{data.why_it_matters.rank}</div>
            ...
            <div className="p-3 space-y-3">
              {Object.entries(data.factor_contributions).map(([factor, val]) => (
                <div key={factor}>
                  ...
                  <div className="h-1.5 bg-brand-base overflow-hidden">
